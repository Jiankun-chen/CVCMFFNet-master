...
def loss_calc(logits, labels):
    ...
    # labels = tf.one_hot(labels, 3)              #对于RGB标签
...
def iou_and_pa(logits, labels):
    ...
    pa_0 = tf.reduce_mean(intersection_0) / tf.reduce_mean(truth_0)
    pa_1 = tf.reduce_mean(intersection_1) / tf.reduce_mean(truth_1)
    pa_2 = tf.reduce_mean(intersection_2) / tf.reduce_mean(truth_2)

    tf.summary.scalar('iou_class0', iou_class0)
    tf.summary.scalar('iou_class1', iou_class1)
    tf.summary.scalar('iou_class2', iou_class2)

    return iou_class0, iou_class1, iou_class2, pa_0, pa_1, pa_2
